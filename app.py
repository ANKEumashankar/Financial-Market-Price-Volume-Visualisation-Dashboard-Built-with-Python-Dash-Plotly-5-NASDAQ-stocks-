...
volume_fig.update_layout(title="Volume Chart", template="plotly_dark", xaxis_title="Date", yaxis_title="Volume (USD $)", )
...
heat_fig.update_layout(title="Correlation Heatmap — All 5 Stocks (Jan 2023 to Jan 2025)", template="plotly_dark")

vol_fig = go.Figure()
for ticker, df in data.items():
    vol_fig.add_trace(go.Scatter(x=df["Date"], y=df["Volatility"], mode="lines", name=ticker))
vol_fig.update_layout(title="Volatility Indicator", template="plotly_dark", xaxis_title="Date", yaxis_title="Volatility (%)")
...
for ticker, df in data.items():
    ...
    for _, row in df.iterrows():
        ...
        if row["Volume"] > avg_vol * 3:
            alerts.append(f"🚨 EXTREME VOL {ticker} {str(row['Date'])[:10]} — Volume {int(row['Volume']/1e6):.0f}M shares — Unusually extreme activity!")
        elif row["Volume"] > avg_vol * 2:
...
.tkr-inner{display:flex;flex-direction:row;flex-wrap:nowrap;white-space:nowrap;animation:tkrMove 30s linear infinite;width:max-content;align-items:center;}
.tkr-inner:hover{animation-play-state:paused;}
.tkr-card{display:inline-flex;align-items:center;gap:8px;padding:0 14px;margin:0 5px;height:36px;background:linear-gradient(135deg,#0d1e30,#0a1628);border:1px solid #1a5276;border-radius:8px;cursor:pointer;flex-shrink:0;white-space:nowrap;}
.tkr-card:hover{background:#1a3a5a;border-color:#00b4ff;}
#fix-r{position:fixed;top:10px;right:15px;z-index:2147483647;padding:5px 14px;border-radius:8px;border:1px solid #1a5276;background:rgba(13,30,48,0.95);color:white;font-size:12px;cursor:pointer;font-weight:600;}
#fix-t{position:fixed;top:44px;right:15px;z-index:2147483647;padding:5px 14px;border-radius:8px;border:1px solid #1a5276;background:rgba(13,30,48,0.95);color:white;font-size:12px;cursor:pointer;font-weight:600;}
</style>
<script>
var _dk=true;
function _tog(){
  _dk=!_dk;
  var t=document.getElementById('fix-t');
  var m=document.getElementById('main-layout')||document.getElementById('main-bg')||document.body;
  if(!_dk){m.style.background='#f0f4f8';t.textContent='🌙 Dark';t.style.background='rgba(255,215,0,0.9)';t.style.color='#1a1a1a';}
  else{m.style.background='#050a0f';t.textContent='☀️ Light';t.style.background='rgba(13,30,48,0.95)';t.style.color='white';}
}
</script>
</head>
<body>
<button id="fix-r" onclick="window.location.reload()">🔄 Refresh</button>
<button id="fix-t" onclick="_tog()">☀️ Light</button>
{%app_entry%}
...
app.layout = html.Div(id="main-layout", style={"backgroundColor": "#050a0f", "fontFamily": "Space Grotesk, Arial", "minHeight": "100vh"}, children=[
        ...
        html.Div([
            ...
            html.H1("FINANCIAL MARKET DASHBOARD",
                style={"fontSize": "28px", "fontWeight": "700", "letterSpacing": "4px",
                       ...
                       "margin": "0"}),
            html.P("Real-Time Price & Volume Analytics — Jan 2023 to Jan 2025",
    ...
    html.Div([
        html.Span(id="refresh-btn", style={"display":"none"}),
        html.Span(id="refresh-msg"),
        html.Span(id="theme-btn", style={"display":"none"}),
    ...
    html.Div([
        # Depth chart on left
        html.Div([dcc.Graph(id="order-book")], style={"width":"60%"}),
        # Order book table on right
        html.Div([
            html.H4("Order Book", style={"color":"#00b4ff","textAlign":"center","letterSpacing":"2px","marginBottom":"8px","fontSize":"14px"}),
            html.Div(id="ob-table", style={"overflowY":"auto","maxHeight":"460px"})
        ], style={"width":"38%","background":"#070d1a","border":"1px solid #1a3a5a","borderRadius":"10px","padding":"12px","marginLeft":"2%"})
    ], style={"display":"flex","alignItems":"flex-start","margin":"20px"}),
    dcc.Graph(id="rsi-chart", style={"margin": "20px"}),
    dcc.Graph(id="volume-profile", style={"margin": "20px"}),
    dcc.Graph(id="vol-price-range", style={"margin": "20px"}),
    html.Div([
        html.H3("Stock Comparison", style={"color":"#00b4ff","textAlign":"center","letterSpacing":"3px","fontSize":"18px","marginBottom":"10px"}),
        html.Div([
            html.Div([
                html.Label("Stock 1:", style={"color":"#7ec8ff","fontSize":"13px","marginRight":"8px"}),
                dcc.Dropdown(id="compare-stock1", options=[{"label":t,"value":t} for t in ["AAPL","TSLA","MSFT","AMZN","GOOGL"]], value="AAPL", clearable=False, style={"width":"140px","display":"inline-block"}),
            ], style={"display":"inline-flex","alignItems":"center","marginRight":"20px"}),
            html.Div([
                html.Label("Stock 2:", style={"color":"#7ec8ff","fontSize":"13px","marginRight":"8px"}),
                dcc.Dropdown(id="compare-stock2", options=[{"label":t,"value":t} for t in ["AAPL","TSLA","MSFT","AMZN","GOOGL"]], value="TSLA", clearable=False, style={"width":"140px","display":"inline-block"}),
            ], style={"display":"inline-flex","alignItems":"center","marginRight":"20px"}),
            html.Div([
                html.Label("Period:", style={"color":"#7ec8ff","fontSize":"13px","marginRight":"8px"}),
                dcc.Dropdown(id="compare-period", options=[{"label":"1 Month","value":30},{"label":"3 Months","value":90},{"label":"6 Months","value":180}], value=90, clearable=False, style={"width":"140px","display":"inline-block"}),
            ], style={"display":"inline-flex","alignItems":"center"}),
        ], style={"display":"flex","justifyContent":"center","flexWrap":"wrap","gap":"10px","marginBottom":"10px"}),
        dcc.Graph(id="compare-chart"),
        html.Div(id="compare-summary", style={"display":"flex","justifyContent":"center","gap":"20px","flexWrap":"wrap","padding":"10px"}),
    ], style={"background":"#070d1a","border":"1px solid #1a3a5a","borderRadius":"12px","padding":"20px","margin":"20px"}),
    html.Div([
...
def update_heatmap(selected):
    ...
    fig2.update_layout(title=f"Correlation Heatmap — How closely stocks move together | {selected} highlighted", template="plotly_dark")
    return fig2
...
def update_chart(days, ticker, n_intervals, play_clicks):
    ...
    fig.update_layout(
        title=f"{ticker} — Candlestick Chart | Open High Low Close | SMA 20 & SMA 50 | Live Price",
...
def update_volume_profile(days, selected):
    ...
    fig.update_layout(title=f"Volume Profile — {selected} highlighted", template="plotly_dark", xaxis_title="Total Volume", yaxis_title="Price Range", )
    return fig
...
def update_volume_chart(days, selected):
    ...
    # Plot all 5 stocks as lines
    for ticker in TICKERS:
        df_t = data[ticker].tail(days).copy()
        df_t["VolumeDollar"] = df_t["Volume"] * df_t["Close"]
        opacity = 1.0 if ticker == selected else 0.3
        fig.add_trace(go.Scatter(
            x=df_t["Date"], y=df_t["VolumeDollar"],
            mode="lines", name=ticker, opacity=opacity,
            line=dict(width=2 if ticker == selected else 1),
            hovertemplate=f"<b>{ticker}</b><br>Date: %{{x}}<br>Volume: $%{{y:,.0f}}<extra></extra>"
        ))

    # Use selected stock for SD bands — 2 years full data
    sel_full = data[selected].copy()
    sel_full["VolumeDollar"] = sel_full["Volume"] * sel_full["Close"]
    sel_tail = sel_full.tail(days).copy()

    mean_vol = sel_tail["VolumeDollar"].mean()
    std_vol  = sel_tail["VolumeDollar"].std()

    sd_above = mean_vol + std_vol
    sd_below = mean_vol - std_vol
    sd_below = max(sd_below, 0)

    dates = sel_tail["Date"]

    # RED line = Mean
    fig.add_trace(go.Scatter(
        x=dates, y=[mean_vol]*len(dates),
        mode="lines", name=f"Mean (Avg Volume)",
        line=dict(color="#ff4444", width=2, dash="dash"),
        hovertemplate=f"Mean Volume: ${mean_vol:,.0f}<extra></extra>"
    ))

    # YELLOW line = SD above mean
    fig.add_trace(go.Scatter(
        x=dates, y=[sd_above]*len(dates),
        mode="lines", name=f"SD Above Mean (+1 SD)",
        line=dict(color="#ffd700", width=2, dash="dot"),
        hovertemplate=f"SD Above: ${sd_above:,.0f}<extra></extra>"
    ))

    # GREEN line = SD below mean
    fig.add_trace(go.Scatter(
        x=dates, y=[sd_below]*len(dates),
        mode="lines", name=f"SD Below Mean (-1 SD)",
        line=dict(color="#00ff99", width=2, dash="dot"),
        hovertemplate=f"SD Below: ${sd_below:,.0f}<extra></extra>"
    ))

    # Shade the SD band area
    fig.add_trace(go.Scatter(
        x=dates.tolist() + dates.tolist()[::-1],
        y=[sd_above]*len(dates) + [sd_below]*len(dates),
        fill="toself",
    ...
    fig.update_layout(
        title=f"Daily Trading Volume (USD $) — {selected} highlighted | Red = Mean | Yellow = +1SD | Green = -1SD | Mean: ${mean_vol/1e9:.2f}B | +1SD: ${sd_above/1e9:.2f}B | -1SD: ${sd_below/1e9:.2f}B",
...
def update_volatility(days, selected):
    ...
    fig.update_layout(title=f"Volatility — {selected} highlighted", template="plotly_dark", xaxis_title="Date", yaxis_title="Volatility (%)")
    return fig
...
def update_order_book(days, selected):
    import numpy as np
    df_sel = data[selected].tail(days)
    price      = round(df_sel["Close"].iloc[-1], 2)
    last_vol   = df_sel["Volume"].iloc[-1]
    volatility = (df_sel["High"] - df_sel["Low"]).mean()
    spread     = round(max(0.05, volatility * 0.001), 2)
    base       = max(500, int(last_vol / 200))
    levels     = 15

    # Generate bid and ask price levels
    bid_prices = [round(price - j * spread, 2) for j in range(1, levels+1)]
    ask_prices = [round(price + j * spread, 2) for j in range(1, levels+1)]

    # Order sizes — larger near current price, decay outward
    bid_sizes = [max(100, int(base * np.exp(-j * 0.15))) for j in range(levels)]
    ask_sizes = [max(100, int(base * np.exp(-j * 0.15))) for j in range(levels)]

    # Cumulative depth — this is what the step chart shows
    bid_cum = list(np.cumsum(bid_sizes))   # grows as price drops
    ask_cum = list(np.cumsum(ask_sizes))   # grows as price rises

    # Bid: prices go from current DOWN — reverse so chart reads left to right
    bid_prices_r = bid_prices[::-1]
    bid_cum_r    = bid_cum[::-1]

    fig = go.Figure()

    # ── GREEN filled area — Bids (left side)
    fig.add_trace(go.Scatter(
        x=bid_prices_r,
        y=bid_cum_r,
        mode="lines",
        name="Bid (Buy)",
        line=dict(color="#00cc66", width=2, shape="hv"),
        fill="tozeroy",
        fillcolor="rgba(0,200,100,0.2)",
        hovertemplate="<b>🟢 BID</b><br>Price: $%{x}<br>Cumulative Size: %{y:,}<extra></extra>"
    ))

    # ── RED filled area — Asks (right side)
    fig.add_trace(go.Scatter(
        x=ask_prices,
        y=ask_cum,
        mode="lines",
        ...
        hovertemplate="<b>🔴 ASK</b><br>Price: $%{x}<br>Cumulative Size: %{y:,}<extra></extra>"
    ))

    # Current price vertical line
    max_depth = max(max(bid_cum), max(ask_cum))
    ...
    spread_val = round(ask_prices[0] - bid_prices[-1], 2)

    fig.update_layout(
        title=f"Order Book Depth Chart — {selected}  |  Current: ${price}  |  Spread: ${spread_val}  |  🟢 Bids (Buy)   🔴 Asks (Sell)",
        template="plotly_dark",
        paper_bgcolor="#070d1a",
        plot_bgcolor="#070d1a",
        height=460,
        ...
        yaxis=dict(
            title="Cumulative Order Size (Shares)",
            gridcolor="#0f2a40",
            showgrid=True
        ),
        legend=dict(orientation="h", yanchor="bottom", y=1.02),
...
def update_rsi(ticker, days):
    ...
    fig.update_layout(
        title=f"{ticker} — RSI Indicator (14 Days) | Overbought > 70 | Oversold < 30",
        template="plotly_dark",
        xaxis_title="Date",
...
@app.callback(
    ...
    Input("compare-stock1","value"),
    Input("compare-stock2","value"),
    Input("compare-period","value"))
def update_comparison(s1, s2, days):
    import plotly.graph_objects as go
    df1 = data[s1].tail(days).copy()
    df2 = data[s2].tail(days).copy()

    # Normalise to % change from start
    df1["pct"] = ((df1["Close"] - df1["Close"].iloc[0]) / df1["Close"].iloc[0]) * 100
    df2["pct"] = ((df2["Close"] - df2["Close"].iloc[0]) / df2["Close"].iloc[0]) * 100

    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df1["Date"], y=df1["pct"], name=s1, line=dict(color="#00b4ff", width=2.5)))
    fig.add_trace(go.Scatter(x=df2["Date"], y=df2["pct"], name=s2, line=dict(color="#ff9500", width=2.5)))

    # Shade difference area
    fig.add_trace(go.Scatter(x=df1["Date"].tolist()+df2["Date"].tolist()[::-1],
        y=df1["pct"].tolist()+df2["pct"].tolist()[::-1],
        fill="toself", fillcolor="rgba(0,180,255,0.08)", line=dict(color="rgba(0,0,0,0)"), showlegend=False))

    fig.update_layout(
        title=dict(text=f"{s1} vs {s2} — % Return Comparison", font=dict(size=14)),
        template="plotly_dark", paper_bgcolor="#070d1a", plot_bgcolor="#070d1a",
        xaxis_title="Date", yaxis_title="% Return from Start",
        legend=dict(orientation="h", yanchor="bottom", y=1.02),
        yaxis=dict(ticksuffix="%"), height=380,
        margin=dict(l=60, r=30, t=80, b=50)
    )
    ...
    g2 = df2["pct"].iloc[-1]
    diff = abs(g1 - g2)
    winner = s1 if g1 > g2 else s2
    loser  = s2 if g1 > g2 else s1
    wg = g1 if g1 > g2 else g2
    lg = g2 if g1 > g2 else g1

    card_style = {"background":"#0d1e30","border":"1px solid #1a5276","borderRadius":"10px",
                  "padding":"14px 24px","textAlign":"center","minWidth":"160px"}
    
    cards = [
        html.Div([
            html.P(s1, style={"color":"#00b4ff","fontSize":"13px","margin":"0","fontWeight":"700"}),
            html.P(f"{g1:+.2f}%", style={"color":"#00ff99" if g1>=0 else "#ff4444","fontSize":"24px","fontWeight":"700","margin":"4px 0 0"}),
        ], style=card_style),
        html.Div([
            html.P(s2, style={"color":"#ff9500","fontSize":"13px","margin":"0","fontWeight":"700"}),
            html.P(f"{g2:+.2f}%", style={"color":"#00ff99" if g2>=0 else "#ff4444","fontSize":"24px","fontWeight":"700","margin":"4px 0 0"}),
        ], style=card_style),
        html.Div([
            html.P("DIFFERENCE", style={"color":"#ffd700","fontSize":"11px","margin":"0","letterSpacing":"2px"}),
            html.P(f"{diff:.2f}%", style={"color":"#ffd700","fontSize":"24px","fontWeight":"700","margin":"4px 0 0"}),
        ], style=card_style),
        html.Div([
            html.P("WINNER", style={"color":"#00ff99","fontSize":"11px","margin":"0","letterSpacing":"2px"}),
            html.P(f"🏆 {winner}", style={"color":"#00ff99","fontSize":"22px","fontWeight":"700","margin":"4px 0 0"}),
        ], style={**card_style,"border":"1px solid #00ff99"}),
    ]
    return fig, cards


@app.callback(Output("vol-price-range", "figure"), Input("timeframe", "value"), Input("stock-selector", "value"))
def update_vol_price_range(days, selected):
    import numpy as np
    df = data[selected].tail(days).copy()
    ...
    # For each price bin calculate volume stats
    bin_means = []
    bin_highs = []
    bin_lows  = []
    ...
    for i in range(len(bins)-1):
        ...
        subset = df[mask]["Volume"] * df[mask]["Close"]
        if len(subset) > 0:
            bin_means.append(subset.mean())
            bin_highs.append(subset.max())
            bin_lows.append(subset.min())
        else:
            bin_means.append(0)
            bin_highs.append(0)
            bin_lows.append(0)

    overall_mean = np.mean([v for v in bin_means if v > 0])

    # Colour bars — green if above mean, red if below
    colors = ["#00ff99" if v >= overall_mean else "#ff4444" for v in bin_means]

    fig = go.Figure()

    # Main volume bars per price range
    fig.add_trace(go.Bar(
        x=labels, y=bin_means,
        name="Avg Volume (USD $)",
        marker_color=colors,
        hovertemplate="<b>Price Range: %{x}</b><br>Avg Volume: $%{y:,.0f}<extra></extra>"
    ))

    # High volume line
    fig.add_trace(go.Scatter(
        x=labels, y=bin_highs,
        mode="lines+markers", name="High Volume",
        line=dict(color="#ffd700", width=2, dash="dot"),
        hovertemplate="High Volume: $%{y:,.0f}<extra></extra>"
    ))
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate="Low Volume: $%{y:,.0f}<extra></extra>"
    ))

    # Mean line across all price ranges
    fig.add_hline(
        y=overall_mean,
        line_dash="dash", line_color="#ff4444", line_width=2,
        annotation_text=f"Mean: ${overall_mean/1e9:.2f}B",
        annotation_font_color="#ff4444"
    )

    fig.update_layout(
        title=f"Volume Price Range — {selected} | Shows volume traded at each price level | Green = Above Mean | Red = Below Mean",
        template="plotly_dark",
        paper_bgcolor="#070d1a",
        plot_bgcolor="#070d1a",
...
@app.callback(Output("ob-table","children"), Input("timeframe","value"), Input("stock-selector","value"))
def update_ob_table(days, selected):
    import numpy as np
    df_sel = data[selected].tail(days)
    price      = round(df_sel["Close"].iloc[-1], 2)
    last_vol   = df_sel["Volume"].iloc[-1]
    volatility = (df_sel["High"] - df_sel["Low"]).mean()
    spread     = round(max(0.05, volatility * 0.001), 2)
    base       = max(200, int(last_vol / 500))
    levels     = 10

    ask_prices = [round(price + j*spread,2) for j in range(1,levels+1)]
    bid_prices = [round(price - j*spread,2) for j in range(1,levels+1)]
    ask_sizes  = [max(50, int(base * np.exp(-j*0.15))) for j in range(levels)]
    bid_sizes  = [max(50, int(base * np.exp(-j*0.15))) for j in range(levels)]
    ask_cum    = list(np.cumsum(ask_sizes))
    bid_cum    = list(np.cumsum(bid_sizes))

    header = html.Div([
        html.Span("Size", style={"width":"33%","display":"inline-block","color":"#4a6a88","fontSize":"11px","letterSpacing":"1px"}),
        html.Span("Price (USD)", style={"width":"33%","display":"inline-block","color":"#4a6a88","fontSize":"11px","letterSpacing":"1px","textAlign":"center"}),
        html.Span("Total", style={"width":"33%","display":"inline-block","color":"#4a6a88","fontSize":"11px","letterSpacing":"1px","textAlign":"right"}),
    ], style={"padding":"4px 8px","borderBottom":"1px solid #1a3a5a","marginBottom":"4px"})

    # Ask rows — red — highest first
    ask_rows = []
    for i in range(levels-1, -1, -1):
        p = ask_prices[i]
        s = ask_sizes[i]
        t = ask_cum[i]
        ask_rows.append(html.Div([
            html.Span(f"{s:,}", style={"width":"33%","display":"inline-block","color":"#ff6666","fontSize":"12px"}),
            html.Span(f"${p}", style={"width":"33%","display":"inline-block","color":"#ff4444","fontSize":"12px","fontWeight":"700","textAlign":"center"}),
            html.Span(f"{t:,}", style={"width":"33%","display":"inline-block","color":"#ff6666","fontSize":"12px","textAlign":"right"}),
        ], style={"padding":"3px 8px","borderBottom":"1px solid rgba(255,68,68,0.1)",
                  "background":f"linear-gradient(to left, rgba(255,60,60,{min(0.15, t/max(ask_cum)*0.3)}) 0%, transparent 100%)"}))

    # Current price row
    price_row = html.Div([
        html.Span(f"${price}", style={"display":"block","textAlign":"center","color":"white",
            "fontSize":"16px","fontWeight":"700","padding":"6px",
            "background":"rgba(255,255,255,0.1)","borderRadius":"4px","margin":"4px 0"}),
    ], style={"padding":"2px 8px"})

    # Bid rows — green — highest first
    bid_rows = []
    for i in range(levels):
        p = bid_prices[i]
        s = bid_sizes[i]
        t = bid_cum[i]
        bid_rows.append(html.Div([
            html.Span(f"{s:,}", style={"width":"33%","display":"inline-block","color":"#00cc77","fontSize":"12px"}),
            html.Span(f"${p}", style={"width":"33%","display":"inline-block","color":"#00ff99","fontSize":"12px","fontWeight":"700","textAlign":"center"}),
            html.Span(f"{t:,}", style={"width":"33%","display":"inline-block","color":"#00cc77","fontSize":"12px","textAlign":"right"}),
        ], style={"padding":"3px 8px","borderBottom":"1px solid rgba(0,255,100,0.1)",
                  "background":f"linear-gradient(to left, rgba(0,200,100,{min(0.15, t/max(bid_cum)*0.3)}) 0%, transparent 100%)"}))

    return [header] + ask_rows + [price_row] + bid_rows
